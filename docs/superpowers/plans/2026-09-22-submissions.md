...
- `e.stopPropagation()` only stops the event reaching *later* nodes (`window`), so on its own it does not stop the drawer.
- `e.nativeEvent.stopImmediatePropagation()` does stop the drawer.
- Keep both: `stopPropagation` still blocks the page's `window` measure-tool listener, which would otherwise re-arm a gesture once the input is gone.
...
As built, see `components/portal/SubmissionNameEditor.tsx` (commits 8283f6f + 3b82896). The code block in Step 2 below is the original, pre-review version.
