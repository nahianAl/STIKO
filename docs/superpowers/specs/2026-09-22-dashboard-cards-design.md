...
3. **Cards are one fixed size from lg up, and never change shape** (user, 2026-09-22, after seeing it in the browser). Tracks are `repeat(auto-fill, min(220px, 100%))`, so every card is 220 × 176px whatever the column width. Opening or closing a panel changes only how many cards fit in a row, and the cards glide to their new cells. The leftover width stays empty at the end of the row. The first build used fluid `minmax(…, 1fr)` tracks, which stretched every card on every frame of a panel's slide. The handoff's `auto-fit` would also stretch a lone card across the whole column. The grid is `items-start`, so a card keeps its own height instead of matching whichever neighbours a reflow puts beside it. Below lg the panels never resize the grid, so the tracks stay fluid there and a phone gets a full-width card.
