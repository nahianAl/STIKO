...
export async function GET(request: NextRequest) {
  ...
  const rows = await sql`
    SELECT id, file_id AS "fileId", user_id AS "userId", parent_comment_id AS "parentCommentId",
           ...
           snapshot_url AS "snapshotUrl", page_number AS "pageNumber",
...
export async function POST(request: NextRequest) {
  ...
  const { fileId, content, xPosition, yPosition, worldX, worldY, worldZ, parentCommentId, author, snapshotUrl, pageNumber } =
    await request.json();
